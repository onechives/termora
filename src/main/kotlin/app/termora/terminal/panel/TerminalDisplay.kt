...
class TerminalDisplay(
    ...
    companion object {
        ...
        private val availableFontFamilyNames by lazy {
            runCatching { com.formdev.flatlaf.util.FontUtils.getAvailableFontFamilyNames().toSet() }
                .getOrElse { emptySet() }
        }
    }
    ...
    private fun getTerminalFont(): Font {
        val terminal = DatabaseManager.getInstance().terminal
        val fontName = if (availableFontFamilyNames.contains(terminal.font)) terminal.font else Font.MONOSPACED
        return Font(fontName, Font.PLAIN, terminal.fontSize)
    }
    ...
    private fun getFallbackTerminalFont(): Font? {
        ...
        return if (terminal.fallbackFont.isBlank()) {
            ...
            val fontName = if (availableFontFamilyNames.contains(terminal.fallbackFont)) terminal.fallbackFont else Font.MONOSPACED
            Font(fontName, Font.PLAIN, terminal.fontSize)
        }
    }
